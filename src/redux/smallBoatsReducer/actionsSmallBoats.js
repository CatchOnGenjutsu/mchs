...
export function getBoatCardInfo(id) {
  return async (dispatch) => {
    ...
    if (id !== "") {
      ...
      const specMarksReq = await fetch(
        MAIN_URL + PORT + API_GET_BOAT_INFO_SPEC_MARKS + String(id),
      );
      const specMarks = await specMarksReq.json();
      const boatArrestsReq = await fetch(
        MAIN_URL + PORT + API_GET_BOAT_INFO_ARRESTS + String(id),
      );
...
export function addNewBoatInfo(data, boatId, tableType, fileType) {
  switch (tableType) {
    ...
    case "specialMarksTableColumns":
      ...
      return async (dispatch) => {
        const response = await fetch(
          MAIN_URL + PORT + API_ADD_NEW_BOAT_SPEC_MARK + boatId + "/" + userId,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    case "boatArrestsTableColumns":
      return async (dispatch) => {
        const response = await fetch(
          MAIN_URL + PORT + API_ADD_BOAT_INFO_ARRESTS + boatId,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      return async (dispatch) => {
        const response = await fetch(
          MAIN_URL +
            PORT +
            API_ADD_BOAT_INFO_DOCS +
            `?cardid=${boatId}&signature=${fileType}`,
          {
            method: "POST",
            body: data,
          },
...
export function editBoatInfo(data, boatId, tableType) {
  switch (tableType) {
    ...
    case "specialMarksTableColumns":
      ...
      return async (dispatch) => {
        const response = await fetch(
          MAIN_URL + PORT + API_EDIT_BOAT_SPEC_MARK + bsmId + "/" + boatId + "/" + userId,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
