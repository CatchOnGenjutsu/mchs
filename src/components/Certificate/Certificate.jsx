...
import ModalWindowCertificates from "./ModalWindow/ModalWindowCertificates.jsx";
import { lossControl } from "./ModalWindow/inputsLabels";
...
export default function Certificate(props) {
	...
	const handleAddNotes = (e) => {
		...
		setModalWindowInputs(lossControl);
	...
	return (
		<div className={styles.wrapper}>
						...
						return (
							<div key={el.keyTable}>
								...
								<table className="table table-bordered border-secondary bg-white">
									<thead>
										<tr>
											{Object.entries(el.nameColoumn).map((column) => (
												<th
													key={column[0]}
													scope="col"
													className={
														"organ number position fio note".includes(column[0])
															? "col-4 text-center"
															: "col-2 text-center"
													}>
													{column[1]}
									...
									<tbody>
										<tr>
											{Object.keys(el.nameColoumn).map((column) => (
												<td key={column}>тут будут данные</td>
											))}
										</tr>
									</tbody>
								</table>
			...
			{showModalWindow ? (
				<ModalWindowCertificates inputHeaders={modalWindowInputs} />
			) : null}
		</div>
	);
}
