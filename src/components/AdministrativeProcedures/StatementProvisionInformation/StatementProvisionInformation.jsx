...
import {
  ...
  API_DOWNLOAD_FILEREPORT_PROVISION_INFO,
...
function StatementProvisionInformation() {
  ...
  const [errorsFields, setErrorsFields] = useState(
    Object.entries({ ...mainInfo, ...fieldAddressOptions })
      ...
      switch (e.target.id) {
        case "accept": {
          const response = await fetch(
            `${MAIN_URL}${PORT}${API_DOWNLOAD_FILEREPORT_PROVISION_INFO}${String(newData.id)}`,
          );
          if (response.ok) {
  ...
  useEffect(() => {
    async function fetchData() {
      if (idStatement) {
        const responseStatement = await fetch(
          MAIN_URL + PORT + API_GET_STATEMENT_PROVISION_INFORMATION + String(idStatement),
        );
